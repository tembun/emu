...
my sub switch_to_message_or_text {
	...
	my $msg_id = $wanted_headers{"Message-ID: "};
	...
	# and the very same with "In-Reply-To".
	my $in_reply;
	if ($wanted_headers{"In-Reply-To: "} ne "") {
		$in_reply = prepare_header($wanted_headers{"In-Reply-To: "})
	}
	else {
		$in_reply = "";
	}
	...
	my $headers = "$date$from$to$cc$subject$msg_id$in_reply";
	...
	if ($boundary) {
		...
		for my $picked_part (@picked_parts) {
			...
			if (
				...
				$prepared_text = decode("UTF-8", decode_base64($prepared_text)) . "\n";
			}
			if ($single_part) {
